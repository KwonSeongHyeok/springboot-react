...
public class PaymentController {
	...
	private ResponseEntity<?> confirmPayment(Map<String, String> requestBody, String encodeKey){
		...
		headers.set("Authorization", encodeKey);//encryptedApiSecretKey 을 위에서 encodedKey 작성해줬음
...
/*****
 ...
 헤더(headers) : HTTP헤더 정보를 포함 ex) 글자인지 이미지인지 동영상인지 .. 어떤 파일이 들어오는 것인가? 누가보내나?
 HttpEntity<문자열이면 문자열, 숫자면 숫자, 모르겠으면 비워두기> abc = new HttpEntity비워져있어도 상관 없음>("요청 본문", headers);
 ...
 ResponseEntity<String이면 String, Integer면 Integer, 여러값이면 ?, 모르겠으면 비워두기> res = new ResponseEntity("응답본문", headers);
 
 ReQuestEntity (ReQuest = 요청 / Http를 상속받아서 Http 기능에 요청에 대한 기능을 추가로 설정한 Entity)
			: HttpEntity를 상속해, Http 요청에 대한 추가적인 정보를 제공
			URI와 HTTP메서드(GET, POST, PUT, DELETE)를 포함하고 있어, 서버로 오쳥을 보낼 때 주로 사용
 RequestEntity<String이면 String, Integer면 Interger, 여러값이면 ?, 모르겠으면 비워두기> req = new RequestEntity("요청본문", headers);
 ...
 ReQuestEntity		  HttpEntity 상속		Http 요청 전송						URI와 HTTP 메서드 포함
