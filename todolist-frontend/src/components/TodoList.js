    ...
    // 할 일 추가버튼 기능 설정
     ...
     const 할일수정버튼 = (todo, index) => {
        ...
        .then(result => {
            ...
            // 기존할일목록(todoList) 복사해서 새로  추가된 할 일을 더한 다음
            // 새로운 할 일로 업데이트
     ...
     const 할일삭제버튼 = (todoNo, index) => {
        ...
        .then(result => {
            // 만약에 결과가 0이라면 alert 창으로 삭제에 실패하였습니다. 띄워주고 되돌아가기
            if(result === 0) {
                alert('제에 실패하였습니다.');
                return;
            }
            ...
            새로운목록리스트.괄호 안에 작성한 부분 제외하고 목록 새로 작성(내가 선택한 번호, 하나만 삭제);
            */
           setTodoList(newTodoList); // 새로 작성한 목록으로 기존목록에 대체하기
        })
        // 삭데 안될 때 문제 보여주기 왜 문제가 생겼는지 개발자용 콘솔창에서만 보여주는 것
        .catch(e => console.log(e));
     }
